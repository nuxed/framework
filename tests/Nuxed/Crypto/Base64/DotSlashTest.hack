...
class DotSlashTest extends HackTest\HackTest {
  <<HackTest\DataProvider('provideRandomBytes')>>
  public function testRandom(string $random): void {
    $enc = Base64\DotSlash\encode($random);
    expect(Base64\DotSlash\decode($enc))->toBeSame($random);
    $unpadded = Str\trim_right($enc, '=');
    expect(Base64\DotSlash\encode($random, false))->toBeSame($unpadded);
    expect(Base64\DotSlash\decode($unpadded))->toBeSame($random);
  }
