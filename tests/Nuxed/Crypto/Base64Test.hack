...
class Base64Test extends HackTest\HackTest {
  <<HackTest\DataProvider('provideRandomBytes')>>
  public function testRandom(string $random): void {
    $enc = Base64\encode($random);
    expect(Base64\decode($enc))->toBeSame($random);
    expect($enc)->toBeSame(\base64_encode($random));
    $unpadded = Str\trim_right($enc, '=');
    expect(Base64\decode($unpadded))->toBeSame($random);
    expect(Base64\decode($unpadded))->toBeSame(\base64_decode($unpadded));
  }

  public function testEnforceStrictPadding(): void {
    ...
    try {
      ...
      static::fail('Strict padding not enforced');
    } catch (\Exception $ex) {
      expect(Base64\decode($str))
        ->toBeSame(\base64_decode($str));
    }
  }
